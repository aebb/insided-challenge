...
class CommentController extends Controller
{
     ...
     * @Route("/community/{user-id}/{community-id}/conversations/{Comment-id}", name="add.comment.conversation", methods={"POST"})
     */
    public function createAction(ServerRequestInterface $request): ResponseInterface
    {
        $command = $this->validator->validate($this->actionFactory->createCommentAction($request));

        return $this->execute(fn() => $this->service->createComment($command), 201);
    }
}
